...
router.route("/getAll").get(function (req, res, next) {
    GigTransfer.find(function (err, data) {
        if (err) {
            return next(err);
        } else {
            res.json(data);
...
router.route("/updateState/:id").post(function (req, res, next) {
    GigTransfer.findByIdAndUpdate(req.params.id,
        {
            $set: req.body,
        },
        (err, data) => {
            if (err) {
                ...
                if (data) {
                    res.json(data);
                    console.log("GigTransfer State updated successfully")
                } else {
                    res.status(404).json({
                        msg: "No data"
                    })
